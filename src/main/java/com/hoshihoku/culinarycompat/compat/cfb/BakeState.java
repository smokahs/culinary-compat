...
public final class BakeState {
	...
	public static final class Manager {
		...
		public static int getBakeDurationTicks() {
			return Math.max(0, Configs.Common.bakewareDurationTicks);
		}
		...
		public static void setConfirm(ServerPlayer player, ResourceLocation outputId) {
			...
			Network.sendToClient(player, new BakeStatusPacket(BakePhase.CONFIRM, outputId));
		}
		...
		public static void setBaking(ServerPlayer player, ResourceLocation outputId, ItemStack output,
			...
			Network.sendToClient(player, new BakeStatusPacket(BakePhase.BAKING, outputId));
		}
		...
		public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
			...
			Network.sendToClient(sp, new BakeStatusPacket(BakePhase.FINISHED, p.outputId));
		...
		public static void onContainerClose(PlayerContainerEvent.Close event) {
			...
			if (p.phase == BakePhase.BAKING) {
				if (Configs.Common.bakewareRefundOnCancel) {
			...
			Network.sendToClient(sp, new BakeStatusPacket(BakePhase.CANCELLED, p.outputId));
			PENDING.remove(sp.getUUID());
		}
	}
		...
		public static void onStatus(BakePhase newPhase, ResourceLocation newOutputId) {
			...
			switch (newPhase) {
				...
				case FINISHED -> {
					phase = null;
					outputId = null;
					if (Configs.Client.bakeDingSound && Configs.Client.dingVolume > 0.0f) {
						playSound(Sounds.DING.get(), Configs.Client.dingVolume);
					}
				}
				case CANCELLED -> {
					phase = null;
